...
func TestFSMs(t *testing.T) {
	...
	assert.Equal(t, `INSERT INTO bookie.fsm(fsmID, created) VALUES (?,?),(?,?),(?,?) ON DUPLICATE KEY UPDATE created = LEAST(created, VALUES(created))`, query.sql, "sql mismatch")
	...
	assert.Equal(t, len(query.values), 6, "length mismatch")
