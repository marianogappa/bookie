...
func TestTags(t *testing.T) {
	...
	assert.Equal(t, `INSERT INTO bookie.tags(fsmID, fsmAlias, k, v) VALUES (?,?,?,?),(?,?,?,?),(?,?,?,?) ON DUPLICATE KEY UPDATE v = VALUES(v)`, query.sql, "sql mismatch")
	...
	assert.Equal(t, len(query.values), 12, "length mismatch")
